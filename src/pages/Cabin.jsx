...
const Cabin = () => {
    ...
    async function fetchData() {
        let { data: cabin, error } = await supabase.from('cabins').select().eq("id", id)
        return cabin[0];
    }
    async function fetchComments(id) {
        let { data: comments, error } = await supabase.from('comments').select().eq("cabin_id", id)
            ...
            <div className="bg-backgroundColor max-h-fit pl-56 pr-56 pt-24 pb-24 gap-16 flex justify-between" >
                ...
                {openedModal ?
                    <div className='flex flex-col items-center w-full gap-4'>
                        <Carroussel images={[image(data.id, 1), image(data.id, 2),image(data.id, 3),image(data.id, 4),image(data.id, 5)]} />
                    ...
                    <>
                        <div className='w-3/4 mb-24 px-8'>
                            <div className='w-full h-[500px] mb-8 '>
                                <button type='button' className='h-full' onClick={modalControl}>
                                    <div className='flex w-full h-full rounded-2xl overflow-hidden  gap-3 '>
                                        <img src={image(data.id, 1)} alt="" className='w-1/2 h-auto object-cover' />
                                        <div className='w-1/2 flex justify-between gap-y-3 flex-wrap rounded-r-lg overflow-hidden'>
                                            <img src={image(data.id, 2)} alt="" className='h-1/2 w-[48.5%] object-cover' />
                                            <img src={image(data.id, 3)} alt="" className='h-1/2 w-[48.5%] object-cover' />
                                            <img src={image(data.id, 4)} alt="" className='h-1/2 w-[48.5%] object-cover' />
                                            <img src={image(data.id, 5)} alt="" className='h-1/2 w-[48.5%] object-cover' />
                                        </div>
                                    </div>
                                </button>
                            </div>
                            <div className='flex justify-between'>
                                ...
                                <LikeButton cabin={data} user={currentUser} isConnected={isConnected} />
                            </div>
                            ...
                            <div className='mt-24 pb-24'>
                                ...
                                <div>
                                    ...
                                    {isConnected &&
                                        <div className='mt-8 '>
                                            {openedCommentForm ? <CommentForm close={setOpenedCommentForm} user={currentUser} cabin={data} postedComment={changeCommentState} /> :
